...
const AddExistingStudent = (props) => {
  ...
  const [selectedYear, setSelectedYear] = useState(authCtx.currentYear);
    ...
    if (e === "all") {
      fetch(
        ...
        .then((data) => {
          ...
          console.log(unitedDataObject);

          const newArrayWithNoRepeatKeys = [...new Set(keysOfAllStudentsArray)];
          console.log(newArrayWithNoRepeatKeys, unitedDataObject, arrayOfYears);
          for (let key of newArrayWithNoRepeatKeys) {
            ...
            if (authCtx.studentsFromCurrentYear[key]?.lessons[props.date[0]]) {
              ...
              console.log(unitedDataObject[key]);
          ...
          for (let year of arrayOfYears) {
            // newArrayWithNoRepeatKeys.map((key) => {});
    ...
    <div>
      ...
      <ul>
        {students.length !== 0 && students !== "undefined"
          ...
          : "-wygląda na to że wszystkich dodałeś-"}
      </ul>
      <div>
        <button
          onClick={(e) => {
